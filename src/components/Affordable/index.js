...
class Affordable extends Component {
  ...
  getData = async () => {
    try {
      ...
      const affordData = oldData.data.filter(
        data => data.value === "affordable"
      )
      this.setState({
  ...
  delete = async id => {
    try {
      ...
      this.setState({
        affordableData: this.state.affordableData.filter(
          data => data._id !== id
        )
      })
    } catch (err) {
  ...
  render() {
    ...
    return (
      <Container>
        ...
        {this.props.isLogged ? <AffordableData addData={this.addData} /> : null}
        <ChartDiv>
          <ToolKit>
            <Button
              style={{ backgroundColor: "#F4934D", marginTop: "10px" }}
              fullWidth
            >
